...
import 'package:components/widgets/calendar/classes/event.dart';
import 'package:components/widgets/calendar/classes/event_list.dart';
import 'package:components/widgets/calendar/src/calendar_header.dart';
import 'package:components/widgets/calendar/src/default_styles.dart';
import 'package:components/widgets/calendar/src/weekday_row.dart';
...
class CalendarCarousel<T extends EventInterface> extends StatefulWidget {
  ...
  const CalendarCarousel({
    ...
    this.todayBorderColor = Colors.yellow,
    this.todayButtonColor = Colors.yellow,
...
class _CalendarState<T extends EventInterface> extends State<CalendarCarousel<T>> {
  ...
  Widget getDefaultDayContainer(
    ...
    return Center(
      child: AnimatedContainer(
          width: (MediaQuery.of(context).size.width - 182) / 7,
          height: (MediaQuery.of(context).size.width - 182) / 7,
          decoration: ShapeDecoration(
              shape: RoundedRectangleBorder(
                side: BorderSide(
                    width: 1,
                    strokeAlign: BorderSide.strokeAlignCenter,
                    color: (isToday || isNextMonthDay || isPrevMonthDay)
                        ? Colors.transparent
          ...
          child: Center(
              child: Text(
            now.day.toString(),
            style: !(isToday || isNextMonthDay || isPrevMonthDay)
                ? textStyle!.copyWith(color: Theme.of(context).secondaryHeaderColor)
                : textStyle,
          ))),
    );
  }
  ...
  void _setDatesAndWeeks() {
    ...
    for (int cnt = 0;
        0 >= DateTime(minDate.year, minDate.month + cnt).difference(DateTime(maxDate.year, maxDate.month)).inDays;
        cnt++) {
    ...
    for (int cnt0 = 0;
        0 >= minDate.add(Duration(days: 7 * cnt0)).difference(maxDate.add(const Duration(days: 7))).inDays;
        cnt0++) {
